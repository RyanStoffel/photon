...
| `smoke` | macos-latest | Downloads that zip, runs the crash/liveness smoke check, then launches the packaged app through `Scripts/check-native-parity.sh`. The native harness posts real global hotkeys, mouse, typing, arrows, and appearance changes; it asserts process activation policy, status-menu presence, `NSPanel` style/frame, `CGWindow` bounds, compact clipboard lifecycle, and application icon resolution. |
